...
import { TrendingUp, Activity, Building2, Lock, Star, FileSpreadsheet, FileText, ClipboardList, Phone } from 'lucide-react';
...
export default function Page5Market() {
  ...
  function txTypeLabel(type: string) {
    const map: Record<string, string> = {
      ...
      '購入 / 轉讓': `${t('common.txTypes.buy')} / ${t('common.txTypes.transfer')}`,
    };
    return map[type] ?? type;
  }
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-shell-green to-shell-teal text-white rounded-xl p-4 flex items-center gap-4">
        ...
        <div className="flex gap-2 flex-shrink-0">
          <button className="bg-white text-shell-green text-xs font-semibold px-3 py-2 rounded-lg hover:bg-white/90 transition-colors whitespace-nowrap">
            {t('p5.earlyAccessCta')}
          </button>
          <button className="bg-white/20 text-white text-xs font-semibold px-3 py-2 rounded-lg hover:bg-white/30 transition-colors whitespace-nowrap">
            {t('p5.earlyAccessCta2')}
          </button>
        </div>
      </div>
      ...
      {/* Market note */}
      <div className="bg-shell-green/5 border border-shell-green/15 rounded-lg px-4 py-3 text-xs text-gray-600 leading-relaxed">
        {t('p5.marketNote')}
      </div>
      ...
      <div className="grid grid-cols-5 gap-5">
        ...
        <div className="col-span-3 bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-3">{t('p5.trendNote')}</p>
      ...
      {isEnterprise ? (
        <div className="space-y-5">
          ...
          {/* Potential Partners Watchlist */}
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h2 className="text-sm font-semibold text-gray-700">{t('p5.partnersTitle')}</h2>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-xs text-gray-500">
                ...
                {marketData.potentialPartners.map((p) => (
                  <tr key={p.name} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5">
                      <span className="text-xs bg-shell-green/10 text-shell-green font-medium px-2 py-0.5 rounded-full">{p.action}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h2 className="text-sm font-semibold text-gray-700">{t('p5.industryTitle')}</h2>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-xs text-gray-500">
                  ...
                  <tr key={row.industry} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 text-xs">
                      <span className={`px-2 py-0.5 rounded-full font-medium ${
                        row.volume === '高' ? 'bg-green-100 text-green-700' :
                        row.volume === '中高' ? 'bg-emerald-100 text-emerald-700' :
                        'bg-gray-100 text-gray-600'
                      }`}>{demandLabel(row.volume === '中高' ? '中' : row.volume)}</span>
                    </td>
                    <td className="px-4 py-2.5 text-xs text-gray-600">{row.creditType}</td>
                    <td className="px-4 py-2.5 text-xs">
                      <span className={`px-2 py-0.5 rounded-full font-medium ${
                        row.activity === '高' || row.activity === '中高' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'
                      }`}>{row.activity}</span>
                    </td>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button className="flex items-center gap-1.5 px-4 py-2 border border-gray-200 text-gray-600 text-xs font-semibold rounded-lg hover:bg-gray-50">
              <Star size={13} /> {t('p5.ctaTrial')}
            </button>
            <button className="flex items-center gap-1.5 px-4 py-2 border border-gray-200 text-gray-600 text-xs font-semibold rounded-lg hover:bg-gray-50">
              <FileText size={13} /> {t('p5.ctaSample')}
            </button>
          </div>
        </div>
      ) : (
