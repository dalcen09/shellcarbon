...
export default function Page2Operations() {
  ...
  const operations = [
    { id: 'buy', label: t('p2.buyLabel'), sub: t('p2.buySub'), icon: ShoppingCart, color: 'bg-green-50 border-green-200 text-green-700', iconColor: 'text-green-600', desc: t('p2.buyDesc') },
    { id: 'sell', label: t('p2.sellLabel'), sub: t('p2.sellSub'), icon: TrendingDown, color: 'bg-red-50 border-red-200 text-red-700', iconColor: 'text-red-500', desc: t('p2.sellDesc') },
    { id: 'transfer', label: t('p2.transferLabel'), sub: t('p2.transferSub'), icon: ArrowRightLeft, color: 'bg-amber-50 border-amber-200 text-amber-700', iconColor: 'text-amber-500', desc: t('p2.transferDesc') },
    { id: 'declare', label: t('p2.declareLabel'), sub: t('p2.declareSub'), icon: Stamp, color: 'bg-blue-50 border-blue-200 text-blue-700', iconColor: 'text-blue-500', desc: t('p2.declareDesc') },
  ];

  const progressSteps = [
    t('p2.step0'), t('p2.step1'), t('p2.step2'),
    t('p2.step3'), t('p2.step4'), t('p2.step5'),
  ];
  ...
  const creditTypeOptions = [
    t('common.creditTypes.renewable'),
    t('common.creditTypes.efficiency'),
    t('common.creditTypes.forest'),
  ];
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  }`}>{appTypeLabel(app.type)}</span>
                </div>
                <div className="text-sm font-semibold text-gray-800 mt-1">
                  {creditTypeLabel(app.creditType)} · {app.quantity.toLocaleString()} t-CO₂
                </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-shell-green/30">
                  {creditTypeOptions.map((opt) => <option key={opt}>{opt}</option>)}
                </select>
              </div>
              <div>
